...
OPTS=""
if [[ "$(hostname -s)" = "graphbm" ]]; then
    OPTS="${OPTS} --dtype float16"
    #OPTS="${OPTS} --max-model-len 16384"
    OPTS="${OPTS} --max-model-len 2048"
    OPTS="${OPTS} --gpu-memory-utilization 0.95"
fi
...
cmd="${ENV}vllm serve microsoft/Phi-4-reasoning-plus --enable-reasoning --reasoning-parser deepseek_r1${OPTS}"
